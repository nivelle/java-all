...
- 优化 2：索引上的等值查询，向右遍历【按索引排序方向】时且最后一个值【最后是指第一个不满足，毕竟之后都不满足了】不满足等值条件的时候，next-key lock 退化为间隙锁。
...
- 同时根据优化原则2,这是一个等值查询（id=7）,而id=10 不满足查询条件,next-key lock 退化成间隙锁,因此最终加锁的范围是（5，10）
