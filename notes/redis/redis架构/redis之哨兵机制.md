...
2. 哨兵检测主库多久没有响应就提升从库为新的主库,这个时间是可以配置的（**down-after-milliseconds**参数）。配置的时间越短,哨兵越敏感,哨兵集群认为主库在短时间内连不上就会发起主从切换,这种配置很可能因为网络拥塞但主库正常而发生不必要的切换,当然,当主库真正故障时,因为切换得及时,对业务的影响最小。
