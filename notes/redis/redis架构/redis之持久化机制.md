...
##### 风险场景2:AOF重写过程中父进程产生写入的场景(此时，写时复制机制下，如果有大key,会导致在重新申请内存空间的时候阻塞)
...
1. 在执行save或者 bgSave  命令创建一个新的RDB文件时,程序会对数据库中的键进行检查,已经过期的键不会被保存到新创建的RDB文件中.因此,数据库中包含过期键不会对生成新的文件造成影响.
