...
- Exception 和 Error 都是 **继承了 Throwable 类**，在 Java 中只有 Throwable 类型的实例才可以被抛出（throw）或者捕获（catch），它是异常处理机制的基本组成类型
...
![java异常概念.png](../images/java异常类.png)
...
- 异常实例的构造十分昂贵。__这是由于在构造异常实例时，Java 虚拟机便需要生成该异常的栈轨迹（stack trace)__
