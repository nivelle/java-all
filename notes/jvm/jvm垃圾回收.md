...
- 针对老年代的垃圾回收器也有三个：刚刚提到的 **Serial Old 和 Parallel Old，以及 CMS**。Serial Old 和 Parallel Old 都是**标记 -
...
- **将整个堆划分为一个个大小512字节的卡**，并且维护一个卡表，用来存储每张卡的一个标识位。
  这个标识位代表对应的卡是否可能存有指向新生代对象的引用。如果存在认为是脏的。
