...
import com.nivelle.guide.blockChain.transaction.TXInput;
...
public class Blockchain {
    ...
    public static Blockchain createBlockchain(String address) {
         ...
         * 如果没有区块那么从创世块开始
         */
        if (Strings.isBlank(lastBlockHash)) {
            // 创建 coinBase 交易
    ...
    public class BlockchainIterator {
        ...
        public boolean hashNext() {
            if (Strings.isBlank(currentBlockHash)) {
                return false;
            }
    ...
    public void signTransaction(Transaction tx, BCECPrivateKey privateKey) throws Exception {
        ...
        for (TXInput txInput : tx.getInputs()) {
            /**
             *根据交易id知道之前的多笔交易,遍历整个区块链，查找是否存在交易
             */
