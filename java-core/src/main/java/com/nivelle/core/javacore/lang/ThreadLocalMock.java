...
public class ThreadLocalMock {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("thread local没有设置threadLocal值，故get方法，返回默认值：");
        System.out.println(Thread.currentThread().getName() + ":" + threadLocal1.get());
        System.out.println(Thread.currentThread().getName() + ":" + threadLocal2.get());
     ...
     *
     * 实现原理:每个Thread 维护一个 ThreadLocalMap 映射表，这个映射表的 key 是 ThreadLocal实例本身，value 是真正需要存储的 Object。
     *
     * (ThreadLocalMap 的key是弱引用的 ThreadLocal,可以对应多个)
     */
    ...
    /**
     *  使用弱应用的目的:Entry继承WeakReference,使用弱引用，可以将ThreadLocal对象的生命周期和线程生命周期解绑,
    ...
    private static void changeParams() {
        // 方法入口处，设置一个变量和当前线程绑定
        setData(new User(12,Thread.currentThread().getName()));
    ...
    private static void setData(User user) {
        System.out.println("执行当前方法的线程名：" + Thread.currentThread().getName());
    ...
    private static void getAndPrintData() {
        ...
        System.out.println("执行当前方法 get数据，线程名：" + Thread.currentThread().getName() + "，数据为：" + user);
    }
