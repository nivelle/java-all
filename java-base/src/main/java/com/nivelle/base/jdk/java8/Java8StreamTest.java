...
public class Java8StreamTest {
     ...
     * 中间操作只对操作进行了记录，即只会返回一个流，不会进行计算操作，而终结操作是实现了计算操作。中间操作又可以分为无状态（Stateless）与有状态（Stateful）操作，前者是指元素的处理不受之前元素的影响，后者是指该操作只有拿到所有元素之后才能继续下去
     *
     * @param args
     */
    public static void main(String[] args) {
        ...
        //filter
        stringCollectionStream.get().filter((s) -> s.startsWith("a")).forEach(System.err::println);
        //sorted
        stringCollectionStream.get().filter(s -> s.startsWith("b")).sorted().forEach(System.err::println);
        //map:map是一种中间过程操作，借助函数表达式将元素转换成另一种形式。可以使用map将每个对象转换为另一种类型。最终输出的结果类型依赖于你传入的函数表达式。
        stringCollectionStream.get().map(String::toUpperCase).sorted().forEach(System.err::println);
        System.err.println("映射然后去重复:" + stringCollectionStream.get().map(String::toLowerCase).distinct().collect(Collectors.toList()));
        //Match 匹配 //任意一个元素满足
         ...
         * */
        userList.stream().reduce((p1, p2) -> p1.age > p2.age ? p1 : p2).ifPresent(System.err::println);

        //通过给定的函数表达式来处理流中的前后两个元素、或者中间结果与下一个元素。并将最终返回的结果放入Optional
        userList.stream().reduce((s1, s2) -> new User(s1.age, s1.name + "//" + s2.name)).ifPresent(System.err::println);
        ...
        System.out.println("累加ageSum" + ageSum);
        ...
        System.out.println("累加 ageSum1" + ageSum1);
        ...
        Map<Integer, List<User>> personsByAge = persons
                .stream()
